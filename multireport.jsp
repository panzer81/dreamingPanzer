...
String RGSNIDNTNO = request.getParameter("RGSNIDNTNO");
String THBRPSMNNO = request.getParameter("THBRPSMNNO");

String[] lsRGSNIDNTNO = RGSNIDNTNO.split(",");
String[] lsTHBRPSMNNO = THBRPSMNNO.split(",");

for(int i=0; i<lsRGSNIDNTNO.length; i++){
	OOFFile file = oof.addFile("crf.root", "%root%/crf/CLIP_report1.crf");
	oof.addConnectionData("*","oracle1");
	file.addField("RGSNIDNTNO", lsRGSNIDNTNO[i]);
	file.addField("THBRPSMNNO", lsTHBRPSMNNO[i]);
}
